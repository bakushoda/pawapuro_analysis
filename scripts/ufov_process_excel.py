...
class UFOVDataIntegrator:
    def __init__(self, data_dir="data"):
        ...
        self.ufov_file = os.path.join(data_dir, "tiger_2024pawapuro_usefulfieldofviewtest(ja)_summary_2508211238.xlsx")
    ...
    def validate_participant_ids(self):
        ...
        # ufov_dataのparticipant_idがstudent_listに存在するかチェック
        ufov_pids = set(self.ufov_processed['subjectId'].unique())
        
        matched_pids = ufov_pids & valid_participant_ids
        unmatched_pids = ufov_pids - valid_participant_ids
    ...
    def match_data_by_participant_id_and_date(self):
        ...
        for _, ufov_row in self.ufov_processed.iterrows():
            participant_id = ufov_row['subjectId']
        ...
        if len(self.matches_df) > 0:
            ...
            print(f"\nSample matches:")
            print(self.matches_df[['participant_id', 'measurement_wave', 'master_date', 'ufov_date', 'date_diff']].head(10))
